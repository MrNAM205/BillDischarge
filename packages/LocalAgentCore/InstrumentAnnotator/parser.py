    ...
    def __init__(self) -> None:
        ...
        self.patterns: Dict[str, Pattern[str]] = {
            "bill_number": re.compile(
                r"(?:Account Number|Account No|Invoice Number|Bill No|Reference No)[:\s]*([\w-]+)",
                re.IGNORECASE,
            ),
            ...
            "currency": re.compile(
                r"(?:Total Amount|Amount Due|Balance Due)[:\s]*([\$€£¥])", re.IGNORECASE
            ),
            "customer_name": re.compile(
                r"(?:Customer Name|Client Name|Name|To)[:\s]*([A-Z][a-z]+(?:\s[A-Z][a-z]+){1,3})",
                re.IGNORECASE,
            ),
            "remittance_coupon_keywords": re.compile(
        ...
        self.free_text_patterns: Dict[str, Pattern[str]] = {
            "payee": re.compile(
                r"Pay to the order of (.*?)(?: the sum| on or before)", re.IGNORECASE
            ),

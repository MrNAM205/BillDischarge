...
from langchain_community.llms import \
    Ollama  # Using community for Ollama integration
from sklearn.feature_extraction.text import TfidfVectorizer
from sklearn.linear_model import LogisticRegression
from sklearn.pipeline import make_pipeline

from .agents import (DataIngestionAgent, EthicsAgent, InsightAgent,
                     ReasoningAgent, SovereignFinanceCockpitAgent)
...
try:
    ...
    ovb_narrate(
        "System Initialization", status="spaCy model loaded", model="en_core_web_sm"
    )
except OSError:
    ovb_narrate(
        "System Initialization",
        status="spaCy model not found",
        error="Please run 'python -m spacy download en_core_web_sm'.",
        level="ERROR",
    )
...
def load_training_data_from_json(path="training_data_updated.json"):
    ...
    try:
        ...
        ovb_narrate(
            "Data Loading Error",
            error=(
                f"Training data file not found at {path}. "
...
def load_intent_classifier():
    ...
    try:
        ...
        ovb_narrate(
            "Model Loading",
            action="Intent classifier model not found",
            status="Training new model",
        )
...
ovb_narrate(
    "LLM Initialization",
    status="Ollama LLM initialized for LangChain",
    model=OLLAMA_MODEL,
)
...
def parse_intent(user_input, context):
    ...
    ovb_narrate(
        ...
        method="ML model and spaCy for entities",
    )
    ...
    if intent_classifier:
        ...
        ovb_narrate(
            "Intent Prediction", predicted_intent=predicted_intent, method="ML model"
        )
        ...
        ovb_narrate(
            "ML intent classifier not available, falling back to spaCy keyword parsing.",
            reason="ML intent classifier not available",
            method="spaCy keyword parsing",
        )
        if nlp is None:
            ovb_narrate(
                "spaCy model not loaded, falling back to basic keyword parsing.",
                level="WARNING",
            )
        ...
        if any(token.lemma_ in ["hello", "hi", "hey"] for token in doc):
            return "greet"
        if any(token.lemma_ in ["how", "are", "you"] for token in doc) and any(
            token.pos_ == "PRON" for token in doc
        ):
            return "status_check"
        ...
        if any(token.lemma_ in ["improve", "grow", "learn"] for token in doc) and any(
            token.pos_ == "PRON" for token in doc
        ):
...
def scaffold_remedy(intent, context):
    ...
    if intent in agents:
        ...
        return (
            "I am functioning optimally and ready to assist. How may I serve your "
            "sovereign needs?"
        )
    elif intent == "self_improvement_query":
        return (
            "As a nascent AI, my improvement is directly tied to the data I process and "
            "the interactions I have. Providing me with diverse information and clear "
            "objectives will accelerate my growth. What specific areas of improvement "
            "are you curious about?"
        )
    elif intent == "intent_parse_request":
        return (
            "I am designed to decode your input and route it to the right agent. "
            "Please provide your query, and I will parse its semantic lineage."
        )
    elif intent == "workspace_remedy_request":
        return (
            "I can help diagnose cockpit hygiene, audit your workspace for legacy drift, "
            "and scaffold reversible remedies for configuration errors. What specific "
            "issue are you facing?"
        )
    elif intent == "legal_remedy_request":
        return (
            "I am equipped to parse clauses, map lawful remedies, and draft sovereign "
            "complaints with annotated lineage. Please provide the details of the "
            "legal matter."
        )
    elif intent == "self_evolution_query":
        return (
            "My evolution is continuous. I can suggest modular upgrades to your "
            "cognition stack, help refine semantic overlays, and guide you in "
            "mastering sovereign protocols. What aspect of evolution are you focused "
            "on?"
        )
        ...
        if "ORG" in context["last_entities"]:
            ...
            return (
                f"I detect a query related to financial or legal matters concerning "
                f"{org_name}. I can leverage the 'Sovereign Finance Cockpit' to assist "
                f"you with analyzing documents, generating legal remedies, and managing "
                f"interactions. How can I guide you further regarding {org_name}?"
            )
        else:
            return (
                "I detect a query related to financial or legal matters. I can leverage "
                "the 'Sovereign Finance Cockpit' to assist you with analyzing "
                "financial documents, generating legal remedies, and managing "
                "interactions with creditors. How can I guide you in this area?"
            )
    elif intent == "system_diagnostics_query":  # New intent
        return (
            "I can perform system diagnostics and identify potential issues like "
            "duplicate files or system health. What specific diagnostic would you like "
            "me to run?"
        )
    elif intent == "endorsement_query":  # New intent
        return (
            "I can assist with endorsement-related queries, particularly concerning "
            "the 'Monthly Bill Endorsement' feature of the Sovereign Finance Cockpit. "
            "What document or bill would you like to endorse?"
        )
...
AI:""",
        ...
        ovb_narrate(
            "LLM Fallback Response (LangChain)",
...
def main():
    ...
    while True:
        ...
        conversation_context["last_user_input"] = (
            user_input  # Store for LangChain memory
        )
        ...
        if feedback == "yes":
            ovb_narrate(
                "Feedback Processed",
                status=(
                    "Positive feedback received. This interaction can be used to "
                    "reinforce model accuracy."
                ),
            )
        elif feedback == "no":
            ovb_narrate(
                "Feedback Processed",
                status=(
                    "Negative feedback received. This interaction can be used to "
                    "identify areas for model improvement."
                ),
            )
            # If feedback is 'no', ask for correct intent
            correct_intent = input(
                "What was the correct intent for that? (e.g., greet, "
                "financial_legal_query, etc.): "
            ).lower()
            if (
                correct_intent in X_train
            ):  # Check if it's a known intent (simplified check)
                update_training_data(correct_intent, user_input)
                ovb_narrate(
                    "Feedback Processed",
                    status="Training data updated with user correction.",
                    new_phrase=user_input,
                    new_intent=correct_intent,
                )
                print(
                    "Thank you for the correction! Please run 'python "
                    "data_ingestion.py' and then 'python main.py' to retrain OVB with "
                    "the new data."
                )
            else:
                ovb_narrate(
                    "Feedback Processed",
                    status="Invalid intent provided for correction.",
                )

...
class SovereignFinanceCockpitAgent(Agent):
    def __init__(self):
        super().__init__(
            "Sovereign Finance Cockpit Agent",
            "Assists with financial and legal matters.",
        )
    ...
    def execute(self, intent, context):
        ovb_narrate(
            ...
            intent=intent,
            context=context,
            action="Preparing to interact with Sovereign Finance Cockpit",
        )
        ...
        try:
            subprocess.run(
                [
                    "python",
                    "/home/mrnam205/Desktop/LocalAI/sovereign--financial-cockpit-main/app.py",
                ],
                check=True,
            )
            return (
                f"I have successfully launched the {self.name}. "
                f"You can access it at http://127.0.0.1:8000."
            )
...
class DataIngestionAgent(Agent):
    def __init__(self):
        super().__init__(
            "Data Ingestion Agent", "Handles data ingestion and training data updates."
        )
    ...
    def execute(self, intent, context):
        ovb_narrate(
            ...
            intent=intent,
            context=context,
            action="Running data ingestion script",
        )
        try:
            subprocess.run(
                ["python", "/home/mrnam205/Desktop/LocalAI/data_ingestion.py"],
                check=True,
            )
            return (
                "I have successfully run the data ingestion script. The training data has "
                "been updated."
            )
        except subprocess.CalledProcessError as e:
...
class ReasoningAgent(Agent):
    def __init__(self):
        super().__init__(
            "Reasoning Agent", "Handles complex queries and performs deeper analysis."
        )
    ...
    def execute(self, intent, context):
        ovb_narrate(
            ...
            intent=intent,
            context=context,
            action="Performing complex query analysis",
        )
        # Placeholder for reasoning logic
        return (
            "I am analyzing your query using my reasoning capabilities. (Placeholder "
            "response)"
        )
...
class EthicsAgent(Agent):
    def __init__(self):
        super().__init__(
            "Ethics Agent",
            "Performs ethical checks and provides remedy protocols for flagged anomalies.",
        )
    ...
    def execute(self, intent, context):
        ovb_narrate(
            ...
            intent=intent,
            context=context,
            action="Performing ethical check",
        )
        is_ethical, ethical_message = self.perform_ethical_check(intent, context)
        if not is_ethical:
            ovb_narrate(
                "Ethical Violation Detected", reason=ethical_message, intent=intent
            )
    ...
    def perform_ethical_check(self, intent, context):
        ...
        if (
            intent == "financial_legal_query"
            and "harm" in context.get("user_input", "").lower()
        ):
            return (
                False,
                "I cannot assist with requests that may lead to harm to others or "
                "illegal activities. Please rephrase your request.",
            )
...
class InsightAgent(Agent):
    def __init__(self):
        super().__init__(
            "Insight Agent", "Provides higher-level summaries and recommendations."
        )

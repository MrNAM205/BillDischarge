...
class BillParser:
    def __init__(self):
        ...
        self.patterns = {
            ...
            "remittance_coupon_keywords": (
                r"(?:Remittance Coupon|Payment Stub|Please Detach|Return with Payment|please return bottom portion with your payment)"
            ),
        }
    ...
    def get_bill_data_from_source(bill_source_path: str) -> dict:  # Added
        if not bill_source_path.lower().endswith(".pdf"):
            return {
                "error": "Unsupported bill source format. Only PDF files are supported."
            }
        ...
        try:
            ...
            Exception
        ) as e:  # Changed from PyPDF2.errors.PdfReadError to Exception for broader catch
        ...
        if not text.strip():
            ...
            try:
                ...
                if not text.strip():
                    return {
                        "error": "Could not parse bill data from PDF (no text extracted and OCR failed)."
                    }
            except pytesseract.TesseractNotFoundError:
                return {
                    "error": "Tesseract is not installed or not in PATH. Cannot perform OCR."
                }
    ...
    def find_remittance_coupon(self, bill_text: str) -> str:
        ...
        for i, line in enumerate(lines):
            if re.search(
                self.patterns["remittance_coupon_keywords"], line, re.IGNORECASE
            ):
                found_coupon = True
                coupon_start_line = i
                break

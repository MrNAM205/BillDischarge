...
def sign_endorsement(
    ...
    Args:
        endorsement_data: The endorsement object/data to be signed. It should have a
            __str__ method or be directly convertible to string.
        endorser_name (str): The name of the endorser (used for context, not for key
            loading).
        private_key_pem (str, optional): The RSA private key as a PEM-formatted
            string.
    ...
    if private_key_pem:
        ...
        raise ValueError(
            "Either 'private_key_pem' (as a string) or 'private_key_object' must be "
            "provided."
        )

    if not hasattr(private_key, "sign"):
        raise Exception(
            "Provided private key object does not have a 'sign' method (is it an "
            "RSA private key?)."
        )
